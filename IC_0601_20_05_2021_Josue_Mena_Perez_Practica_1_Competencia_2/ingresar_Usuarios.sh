...
crearUsuario(){
	...
	then 
		...
		then 
		  ...
		  useradd -u "${UserID}" -g "${GroupID}" -c "${UserIDinf}" -m -d "${HomeDir}" -s "${ComandSell}" "${username}"
		  #Indica la contraseña que se le asignará al usuario.
		  echo "${username}:${password}" | chpasswd --md5
		  #Obliga el vencimiento inmediato de la contraseña.
		  chage -d 0 "${username}"
		  echo "---------------------------------------------------"
		fi
	else
		...
		echo -e "\n**********************Warning**********************"
		echo "El grupo [${GroupID}] no existe."
